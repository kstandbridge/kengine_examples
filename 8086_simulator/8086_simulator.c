...
typedef enum op_code
{
    ...
    OpCode_OutLoadEAToRegister               = 0b10001101,
    OpCode_OutLoadPointerToDS                = 0b11000101,
    OpCode_OutLoadPointerToES                = 0b11000100,
    ...
    OpCode_StringRepeat                      = 0b1111001,
    OpCode_StringMoveByteWord                = 0b1010010,
    OpCode_StringCompareByteWord             = 0b1010011,
    OpCode_StringScanByteWord                = 0b1010111,
    OpCode_StringLoadByteWdToALXL            = 0b1010110,
    OpCode_StringStoreByteWdFromALXL         = 0b1010101,
    
    OpCode_InFixedPort                       = 0b1110010,
    OpCode_InVariablePort                    = 0b1110110,
    
    OpCode_OutFixedPort                      = 0b1110011,
    OpCode_OutVariablePort                   = 0b1110111,
    
    OpCode_PushRegisterMemory                = 0b11111111,
    OpCode_PushRegister                      = 0b01010,
    
    OpCode_PopRegisterMemory                 = 0b10001111,
    OpCode_PopRegister                       = 0b01011,
    
    OpCode_XchgRegisterWithMemory            = 0b10010,
    
    OpCode_IncRegisterMemory                 = 0b1111111,
    OpCode_IncRegister                       = 0b01000,
    
    OpCode_DecRegister                       = 0b01001,
    OpCode_DecNegChangeSign                  = 0b1111011,
    
    OpCode_BusLockPrefix                     = 0b11110000,
    
} op_code;

inline string
OpCodeToString(op_code Code)
{
    string Result;
    
    switch(Code)
    {
        case OpCode_OutLoadEAToRegister:             { Result = String("lea"); } break;
        case OpCode_OutLoadPointerToDS:              { Result = String("lds"); } break;
        case OpCode_OutLoadPointerToES:              { Result = String("les"); } break;
        default:                                     { Result = String("TODO OpCodeToString"); } break;
    }
...
inline void
RegisterMemory(format_string_state *State, u8 **At, umm *Index, u8 Op0, b32 Segment, string SegPrefix)
{
    u8 Op1 = (*At)[(*Index)++];
    u8 Mod = Op1 >> 6;
    ...
    string Op;
    if(Op0 == OpCode_PopRegisterMemory)
    {
        Op = String("pop");
    }
    else if(((Op0 >> 0) == OpCode_PushRegisterMemory) ||
            ((Op0 >> 1) == OpCode_IncRegisterMemory))
    {
        string Ops[] = { String("inc"), String("dec"), String("call"), String("regs3"), String("jmp"), String("regs2"), String("push"), String("opsss") };
        Op = Ops[Src];
    }
    else if((Op0 >> 1) == OpCode_DecNegChangeSign)
    {
        string Ops[] = { String("test"), String("ops1"), String("not"), String("neg"), String("mul"), String("imul"), String("div"), String("idiv"), String("opss") };
        Op = Ops[Src];
    }
    else
    {
        Op = OpCodeToString(Op0);
    }
    
    b32 IsWord = (Op0 & 1);
    if(RegisterMode)
    {
        if(((Op0 >> 1) == OpCode_DecNegChangeSign) && 
           (Src == 0b000)) // test
        {
            AppendFormatString(State, "%S %S, %d", Op, GetRegisterName(Dest, IsWord), (*At)[(*Index)++]);   
        }
        else
        {
            AppendFormatString(State, "%S %S", Op, GetRegisterName(Dest, IsWord));   
        }
    }
    else if(NoDisplacement)
    {
        ...
        if(DirectAccess)
        {
            ...
            {
                AppendFormatString(State, "%S %S [%d]", Op, IsWord ? String("word") : String("byte"), Value);
            }
        }
        else
        {
            AppendFormatString(State, "%S %S ", Op, IsWord ? String("word") : String("byte"));
            if(Segment)
            {
                AppendFormatString(State, "%S:", SegPrefix);
            }
            AppendFormatString(State, "[%S]", Bytes[Dest]);
            if(((Op0 >> 1) == OpCode_DecNegChangeSign) && 
               (Src == 0b000)) // test
            {
    ...
    else
    {
        ...
        if((Op0 == OpCode_OutLoadEAToRegister) ||
           (Op0 == OpCode_OutLoadPointerToDS) ||
           (Op0 == OpCode_OutLoadPointerToES))
        {
            AppendFormatString(State, "%S, ", GetRegisterName(Src, 1));
        }
        else if(((Op0 >> 1) == OpCode_DecNegChangeSign) || 
                (Src != 0b010))
        {
            if(Op0 & 1)
            {
                AppendFormatString(State, "word ");
            }
            else
            {
                AppendFormatString(State, "byte ");
            }
        }
        ...
        Index < Size;)
    {
        ...
        {        
            ...
            switch(Op0)
            {
                ...
                case OpCode_OutLoadEAToRegister:
                case OpCode_OutLoadPointerToDS:
                case OpCode_OutLoadPointerToES:
                case OpCode_PushRegisterMemory:
                case OpCode_PopRegisterMemory:
                {
                    RegisterMemory(&State, &At, &Index, Op0, Segment, SegPrefix);
                } break;
                ...
                default:
                {
                    switch(Op0 >> 1)
                    {
                        case OpCode_IncRegisterMemory:
                        case OpCode_DecNegChangeSign:
                        {
                            RegisterMemory(&State, &At, &Index, Op0, Segment, SegPrefix);
                        } break;
                        
                        case OpCode_StringRepeat:
                        {
                            u8 Op1 = At[Index++];
                            b32 IsWord = (Op1 & 1);
                            ...
                            switch(Op1 >> 1)
                            {
                                case OpCode_StringMoveByteWord:        { AppendFormatString(&State, "rep movs%S", W); } break;
                                case OpCode_StringCompareByteWord:     { AppendFormatString(&State, "rep cmps%S", W); } break;
                                case OpCode_StringScanByteWord:        { AppendFormatString(&State, "rep scas%S", W); } break;
                                case OpCode_StringLoadByteWdToALXL:    { AppendFormatString(&State, "rep lods%S", W); } break;
                                case OpCode_StringStoreByteWdFromALXL: { AppendFormatString(&State, "rep stos%S", W); } break;
                                default:                               { AppendFormatString(&State, "TODO REP %b", Op1); }
                            }
                        } break;
                        
                        case OpCode_InFixedPort:    { AppendFormatString(&State, "in al, %u", At[Index++]); } break;
                        case OpCode_InVariablePort: { AppendFormatString(&State, "in %S, dx", (Op0 & 1) ? String("ax") : String("al")); } break;
                        
                        case OpCode_OutFixedPort:    { AppendFormatString(&State, "out %u, ax", At[Index++]); } break;
                        case OpCode_OutVariablePort: { AppendFormatString(&State, "out dx, %S", (Op0 & 1) ? String("ax") : String("al")); } break;
                        
                        default:
                        {
                            switch(Op0 >> 3)
                            {
                                case OpCode_PushRegister:            { AppendFormatString(&State, "push %S", GetRegisterName(Op0 & 0b111, 1)); } break;
                                case OpCode_PopRegister:             { AppendFormatString(&State, "pop %S", GetRegisterName(Op0 & 0b111, 1)); } break;
                                
                                case OpCode_XchgRegisterWithMemory:  { AppendFormatString(&State, "xchg ax, %S", GetRegisterName(Op0 & 0b111, 1)); } break;
                                
                                case OpCode_IncRegister:             { AppendFormatString(&State, "inc %S", GetRegisterName(Op0 & 0b111, 1)); } break;
                                
                                case OpCode_DecRegister:             { AppendFormatString(&State, "dec %S", GetRegisterName(Op0 & 0b111, 1)); } break;
