        ...
        ++Index)
    {
        ...
        else
        {
            ...
            {            
                ...
                if(Op == TextOp_Draw)
                {
                    PushRenderCommandGlyph(RenderGroup, GlyphOffset, Depth, Size, Color, Info->UV, AppState->SpriteTexture);
                }
                else
                {
...
InitApp(app_memory *AppMemory)
{
    ...
    // NOTE(kstandbridge): Loading a glyph sprite sheet
    {
        ...
        AppState->GlyphTexture = DirectXLoadTexture(TotalWidth, TotalHeight, TextureBytes);
        
    }
    
#if 0    
    // NOTE(kstandbridge): Loading the sprint sheet
    {
        ...
        u8 *TextureBytes = stbi_load("minesweeper\\minesweeper-sprites.png", &TotalWidth, &TotalHeight,
                                     &TextureChannelCount, TextureForceChannelCount);
        Assert(TextureBytes);
        
        AppState->SpriteTexture = DirectXLoadTexture(TotalWidth, TotalHeight, (u32 *)TextureBytes);
    }
#endif
...
#define BOX_PADDING 3
    ...
    {    
        v2 P = V2(500, 400);
        v2 Size = V2(81, 115);
        PushRenderCommandSprite(RenderGroup, P, 3.0f, Size, V4(1, 1, 1, 1), V4(0, 0, 1, 1), AppState->SpriteTexture);
    }
    ...
    {    
        ...
        PushRenderCommandGlyph(RenderGroup, P, 3.0f, Size, V4(0, 0, 0, 1), V4(0, 0, 1, 1), AppState->GlyphTexture);
    }
    
    {    
        v2 P = V2(10 + AppState->SpriteSheetSize.X, 0);
        v2 Size = AppState->SpriteSheetSize;
        PushRenderCommandSprite(RenderGroup, P, 3.0f, Size, V4(0, 0, 0, 1), V4(0, 0, 1, 1), AppState->GlyphTexture);
    }
